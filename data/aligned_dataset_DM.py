...
class AlignedDatasetDM(BaseDataset):   ## train
    ...
    def __getitem__(self, index):
        ...
        return {'A': A, 'B': B,
            'A_paths': AB_path, 'B_paths': AB_path,'time_ratio': time_ratio}
...
class TestAlignedDatasetDM(BaseDataset):
    def initialize(self, opt):
        self.opt = opt
        self.root = opt.dataroot
        self.dir_AB = os.path.join(opt.dataroot, opt.phase)
    ...
    def __getitem__(self, index):
        ...
        # Extract Time Period from filename
        #time_ratio = float(AB_path.split('_')[-1].split('.')[0][:-1])
